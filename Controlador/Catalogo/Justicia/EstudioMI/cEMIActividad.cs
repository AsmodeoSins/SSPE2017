...
{
    public class cEMIActividad:EntityManagerServer<EMI_ACTIVIDAD>
    {
        #region Constructors
        public cEMIActividad() { }
        #endregion
        #region Obtener
        /// <summary>
        ...
        public IQueryable<EMI_ACTIVIDAD> Obtener()
        {
            try
            {
                return GetData();
        ...
        public EMI_ACTIVIDAD Obtener(int idEmi,int idEmiCons, int idConsec)
        {
            return GetData(w=>w.ID_EMI == idEmi&&w.ID_EMI_CONS == idEmiCons&& w.ID_CONSEC == idConsec).SingleOrDefault();
        }
        #endregion
        #region Insercion
        /// <summary>
        ...
        public string Agregar(EMI_ACTIVIDAD Entidad)
        {
            try
            {
        ...
        /// <returns>Cadena de texto con el resultado correspondiente.</returns>
        public bool Insertar(int Id, short Consecutivo, List<EMI_ACTIVIDAD> List)
        {
            try
            {
            ...
            {
                return false;//"Ocurri\u00F3 un error: " + ex.Message;
            }
        }
        #endregion
        ...
        public string Actualizar(EMI_ACTIVIDAD Entidad)
        {
            try
            {
                if (Update(Entidad))
        ...
        /// <returns>Cadena de texto con el resultado de la operación.</returns>
        public string Eliminar(EMI_ACTIVIDAD Entidad)
        {
            try
            {
                if (Delete(Entidad))
