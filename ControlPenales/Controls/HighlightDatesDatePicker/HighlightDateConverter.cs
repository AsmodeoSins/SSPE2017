...
namespace ControlPenales.Controls
{
    public class HighlightDateConverter : IMultiValueConverter
    {
		...
		public object Convert(object[] values, Type targetType, object parameter, System.Globalization.CultureInfo culture)
		{
			...
			var parent = (HighlightDatesDatePicker) values[1];
			
			// Exit if highlighting turned off
			if (parent.ShowDateHighlighting == false) return null;

			// Exit if no HighlightedDateText array
			if (parent.HighlightedDateList == null || parent.HighlightedDateList.Count==0) return null;
            ...
            //La parte de abajo se cambia por nuevo codigo para poder enviar una lista de fechas

            if (parent.HighlightedDateList.Contains(targetDate))
                highlightText=DateTimeExtensions.FechaATexto(targetDate);
