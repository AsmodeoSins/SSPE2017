...
{
    public partial class ReporteProcesadosFueroSexoDelitoViewModel : ValidationViewModelBase
    {
        public ReporteProcesadosFueroSexoDelitoViewModel() { }
        ...
        private void OnLoad(ReporteProcesadosFueroSexoDelitoView Window = null)
        {
            try
            {
                ConfiguraPermisos();
        ...
        public void GenerarReporte()
        {
            try
            {
                ...
                encabezado.Add(new cEncabezado()
                {
                    ...
                    PieUno = string.Format("Reporte de Procesados por Sexo, Fuero y Delito \n Del {0} Al {1}",
                ...
                var PrimerIngreso = _IngresosActivos.Where(x => x.ID_CLASIFICACION_JURIDICA == "2" && x.CAUSA_PENAL.Any(y => y.CAUSA_PENAL_DELITO.Any() && y.ID_ESTATUS_CP == (short)eEstatusCausaPenal.EN_PROCESO || y.ID_ESTATUS_CP == (short)eEstatusCausaPenal.ACTIVO));
                var _CuantosPrimeros = PrimerIngreso.Count();
                foreach (var item in PrimerIngreso)
                {//CAUSAS PENALES CON DELITOS, ACTIVAS, PRODUCTO DE UN INGRESO PROCESADO ACTIVO
        ...
        private void ConfiguraPermisos()
        {
            try
            {
                var permisos = new SSP.Controlador.Catalogo.Justicia.cProcesoUsuario().Obtener(enumProcesos.REPORTE_PROCESADO_SEXO_FUERO_DELITO.ToString(), StaticSourcesViewModel.UsuarioLogin.Username);
