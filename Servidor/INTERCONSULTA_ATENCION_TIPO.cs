//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace SSP.Servidor
{
    using System;
    using System.Collections.Generic;
    
    public partial class INTERCONSULTA_ATENCION_TIPO
    {
        public INTERCONSULTA_ATENCION_TIPO()
        {
            this.CANALIZACION_ESPECIALIDAD = new HashSet<CANALIZACION_ESPECIALIDAD>();
            this.TV_CANALIZACION_ESPECIALIDAD = new HashSet<TV_CANALIZACION_ESPECIALIDAD>();
            this.INTERCONSULTA_SOLICITUD = new HashSet<INTERCONSULTA_SOLICITUD>();
            this.TV_INTERCONSULTA_SOLICITUD = new HashSet<TV_INTERCONSULTA_SOLICITUD>();
        }
    
        public short ID_INTERAT { get; set; }
        public string DESCR { get; set; }
        public string CATALOGO_ESPECIALIDADES { get; set; }
        public string CATALOGO_SERVICIOS_ESP { get; set; }
        public string ESTATUS { get; set; }
    
        public virtual ICollection<CANALIZACION_ESPECIALIDAD> CANALIZACION_ESPECIALIDAD { get; set; }
        public virtual ICollection<TV_CANALIZACION_ESPECIALIDAD> TV_CANALIZACION_ESPECIALIDAD { get; set; }
        public virtual ICollection<INTERCONSULTA_SOLICITUD> INTERCONSULTA_SOLICITUD { get; set; }
        public virtual ICollection<TV_INTERCONSULTA_SOLICITUD> TV_INTERCONSULTA_SOLICITUD { get; set; }
    }
}
